...
These represent different points on the Fidelity spectrum, both demonstrating Clef compiled to efficient native code, but with different runtime environments.
...
Both still benefit from Fidelity's core value proposition: Clef compiled to native code with deterministic memory management. The difference is what sits beneath the application.
...
├─────────────────────────────────────────────────────────────┤
│ Your Clef Application (compiled via Fidelity)                │
│   - main() entry point                                      │
│   - CCS (native Clef stdlib)                       │
│   - Application logic                                       │
├─────────────────────────────────────────────────────────────┤
...
│   - Interrupt handlers                                      │
├─────────────────────────────────────────────────────────────┤
│ Startup Code (generated by Composer)                        │
│   - Vector table                                            │
...
- `-nostdlib`: No standard library - CCS provides everything
...
module Platform.Bindings.USB =
    // USB OTG registers at 0x4000D400
    let private USB_BASE = 0x4000D400n

    /// Initialize USB peripheral
    let init () : unit =
        ()  // Alex emits direct register initialization

    /// Send HID report
    let sendReport (data: nativeint) (len: int) : int =
        Unchecked.defaultof<int>

    /// Receive HID report
    let receiveReport (buffer: nativeint) (maxLen: int) : int =
        Unchecked.defaultof<int>
```

This is statically linked - the USB stack becomes part of the unikernel binary. Alex recognizes `Platform.Bindings.USB.*` calls and emits MLIR for direct register access.
...
```
┌─────────────────────────────────────────────────────────────┐
│ Keystation Application (Fidelity-compiled Clef)              │
...
The Clef application is a **userspace process** running on Linux. It still has:
...
```
Clef Source + CCS + Linux Bindings
        ↓ CCS
    Typed AST
        ↓ PSG Builder
    Program Semantic Graph
...
// Or using libgpiod bindings (Platform.Bindings pattern)
// Farscape generates these from libgpiod headers
module Platform.Bindings.GpioD =
    /// Open GPIO chip device
    let chipOpen (path: nativeint) : nativeint =
        Unchecked.defaultof<nativeint>

    /// Set GPIO line value
    let lineSetValue (line: nativeint) (value: int) : int =
        Unchecked.defaultof<int>
```
...
| Region | Size | Usage |
|--------|------|-------|
| Code | ~2-4 MB | Application, CCS, LVGL |
...
Farscape generates Platform.Bindings (BCL-free pattern) that Alex recognizes and provides platform-specific MLIR emission.

For **unikernel** targets, bindings become direct register access:
```fsharp
// Platform.Bindings for HAL - Alex emits register manipulation
module Platform.Bindings.HAL.GPIO =
    let writePin (port: nativeint) (pin: uint16) (state: int) : unit =
        ()  // Alex emits BSRR register write
```

For **Linux** targets, bindings become syscalls or library calls:
```fsharp
// Platform.Bindings for Linux GPIO - Alex emits libgpiod calls
module Platform.Bindings.GpioD =
    let lineSetValue (line: nativeint) (value: int) : int =
        Unchecked.defaultof<int>

// Platform.Bindings.writeBytes via CCS - Alex emits write syscall
```
...
**Clef code with deterministic memory, compiled to native performance.**
...
- [x] PSG captures full semantic graph from CCS
...
### SpeakEZ Blog Posts
- "Clef Goes Metal: Fidelity's Hardware/Software Co-Design Revolution"
- "The Farscape Bridge"
- "Binding Clef to C++ in Farscape"
