...
| Generated library graph | Which library owns each public Clef declaration, how consumers reference it, and the supported target representation. |
...
Establish these relationships during ingestion and semantic enrichment. Alex consumes established facts. It does not infer ownership from a symbol spelling, choose a constructor from the first interface-shaped edge or repair generic constraints at emission.
...
At commitment, each required property needs sufficient evidence, a sound generated boundary check, or an explicit external premise permitted by the contract. Contradictions and unavailable capabilities are located errors. A solver timeout or unknown result is not success.
...
3. **Clef contract.** Review candidate bindings or source, developer-supplied assumptions, supported witnessing rules and outstanding constraints. Type-check actual producer and consumer libraries together.
