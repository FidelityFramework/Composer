...
**Status (2026-09-05).** Every row is computed in CCS. Composer's former `PSGElaboration` layer is gone (`clef/docs/fidelity/phg/Dimensional_Range_Design.md`, "Where code lives"): escape, curried-chain normalisation and partial applications, meets, closure and aggregate placement, platform call-site resolution, pins and the declaration roots are saturation passes writing `SemanticGraph.Codata`, beside `Layouts`, `FieldRanges`, `ElementRanges` and the node's `ValueRange`. There is no SSA pre-assignment: a value is named at emission from its node (`Composer/src/MiddleEnd/Alex/Traversal/Values.fs`), a pure derivation with no pass and no counter. Composer holds no interim computation over the graph, and none is to be added.
