...
- **Road B — hand-roll, sidestep LLVM.** Emit the instruction stream directly from a fully-lowered MLIR dialect. No fork, no generic-IR layer; pays the cost of building instruction selection, register allocation, and binary encoding ourselves.
...
- **Register allocation → physical-register lowering of an existing virtual-SSA coeffect.** Composer *already* computes a virtual-SSA coeffect. [SSAAssignment.fs](../src/MiddleEnd/PSGElaboration/SSAAssignment.fs) assigns each PSG node a structured SSA value — `V of int | Arg of int` ([Types.fs:107-110](../src/MiddleEnd/Alex/Dialects/Core/Types.fs)) — *before* MLIR emission, with the counter resetting at every Lambda boundary so allocation is per-function-scope:

  ```fsharp
  // FunctionScope — per-lambda SSA numbering (SSAAssignment.fs:929-945)
  let yieldSSA (scope: FunctionScope) : SSA * FunctionScope =
      let ssa = V scope.Counter
      ssa, { scope with Counter = scope.Counter + 1 }
  ```

  The module's own docstring states the intent: the counts are "derived from actual node STRUCTURE, not just node KIND ... deterministic function of instance structure" ([SSAAssignment.fs:23-31](../src/MiddleEnd/PSGElaboration/SSAAssignment.fs)), and the result is a `Map<NodeId, NodeSSAAllocation>` that "witnesses read (coeffect lookup, no generation during emission)" ([SSAAssignment.fs:9](../src/MiddleEnd/PSGElaboration/SSAAssignment.fs)). This is *regalloc as mise-en-place* — but only the *virtual* half exists today. **There is no physical-register allocator anywhere in the pipeline.** Physical-register lowering — pinning each `V`/`Arg` to an `x`-register or a spill slot — is **net new work**, and it is the natural extension of the existing coeffect rather than a parallel mechanism. The virtual-SSA coeffect is the soil; physreg lowering is the crop planted in it.
...
### The first fully self-contained codegen

There is one further, genuinely novel cost that the CIRCT and AIE backends do *not* pay, and it must be named plainly. Both of those backends, despite their direct-emit orchestration phases, ultimately *delegate to external tools* — `circt-opt` and the SystemVerilog flow for CIRCT, `aie-opt` plus a per-core compiler and CDO/xclbin packaging for AIE ([Lowering.fs:240](../src/BackEnd/AIE/Lowering.fs)). They have an escape hatch.

A dedicated RISC-V backend on a pure-orchestration substrate has **none**. It goes from fully-lowered MLIR to a **directly-spec'd binary itself** — its own instruction encoder, its own relocation/packaging, its own bit stream. This is Composer's **first fully self-contained code generator**: a bigger step than any prior backend, with no external tool to fall back on. That cost is not a reason to avoid the backend; it is precisely *why* the backend is **gated to material-advantage targets only**. We pay the "build the whole bit stream ourselves" tax exactly where, and only where, avoiding the lossy LLVM handoff is worth it.
...
**SiFive standard cores — least advantage.** A SiFive core *is* the application processor. It has a transparent, hardware-managed cache; a conventional stack; an inherited ABI. This is LLVM's mainline RISC-V target, tuned over years. Two practical facts reinforce keeping LLVM here: such a target wants a managed runtime with **GC** and the **V (vector) extension**, neither of which the `riscv-fs` oracle currently models; and on a transparent-cache core there is no semantic context being thrown away at the handoff that LLVM could not also have used. The correct move is to **keep the triple** and route through `BackEnd.LLVM.Pipeline.backend` exactly as `CPU` does today ([PlatformPipeline.fs:16](../src/Core/PlatformPipeline.fs)).
...
- **Coeffects are the *intent*.** The tiling nanopass, the physreg lowering of the virtual-SSA coeffect, and the ABI coeffect together record *what the program means to do* on the RISC-V core.
...
| **Register allocation** | — | Physical-register lowering of the virtual-SSA coeffect ([SSAAssignment.fs](../src/MiddleEnd/PSGElaboration/SSAAssignment.fs)); no physreg allocator exists today |
...
4. **Register allocation as coeffect** — physical-register lowering of the existing virtual-SSA allocation from [SSAAssignment.fs](../src/MiddleEnd/PSGElaboration/SSAAssignment.fs); this is the net-new physreg step exercised for the first time.
